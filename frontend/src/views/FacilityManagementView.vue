      ...
      <div v-if="showFacilityForm" class="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-end">
        <div class="bg-gray-50 w-full h-screen flex flex-col">
          ...
          <div class="flex-1 overflow-y-auto px-4 py-6 space-y-5">
            ...
            <div v-if="!editingFacility && formData.cost > 0" class="space-y-3">
              ...
              <div class="bg-blue-50 border border-blue-200 rounded-xl p-3">
                ...
                <div v-else class="text-sm text-blue-800">
                  💵 Tiền mặt: <span class="font-bold">{{ formatPrice(fundBalance?.cash || 0) }}</span>
                  &nbsp;|&nbsp;
                  💳 CK: <span class="font-bold">{{ formatPrice(fundBalance?.transfer || 0) }}</span>
                </div>
              </div>
              ...
              <div class="bg-orange-50 border-2 border-orange-300 rounded-xl p-3">
                <div class="flex items-center gap-2 mb-1">
                  ...
                  <span class="text-sm font-semibold text-orange-800">Tự động trừ Quỹ vận hành</span>
                </div>
                <div class="text-xl font-bold text-orange-700">{{ formatPrice(formData.cost) }}</div>
                <div v-if="formData.money_type !== 'transfer' && formData.cost > (fundBalance?.cash || 0)"
                  class="text-xs text-red-600 mt-1 font-semibold">⚠️ Tiền mặt quỹ không đủ!</div>
                <div v-if="formData.money_type === 'transfer' && formData.cost > (fundBalance?.transfer || 0)"
...
const fetchFundBalance = async () => {
  ...
  try {
    ...
    // Get operating fund balance specifically
    fundBalance.value = data?.['operating'] || { cash: 0, transfer: 0, total: 0 }
  } catch (error) {
    fundBalanceError.value = 'Không thể tải số dư quỹ vận hành. Vui lòng thử lại.'
  } finally {
...
const saveFacility = async () => {
  try {
    ...
    if (editingFacility.value) {
      ...
      // Always deduct from operating fund when cost > 0
      if (dataToSend.cost > 0) {
        dataToSend.paid_from_fund = true
      }
